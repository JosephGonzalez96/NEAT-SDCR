...
float* evaluate_network(Genome *genome, unsigned char (*inputs)[length][4],float *results,FILE *file_evaluation){
...
double evaluate_false_fitness(unsigned char (*inputs)[length][4],int outputs, int (*intensities) ,Genome *genome,int (*sentence)[tests_numbers],int tes,FILE *file_evaluation, FILE *file_tests, int generation){
    float neuron_outputs[number_outputs];
    float result_angle;
    int choose_point, fitness = 0;
    evaluate_network(genome,inputs,neuron_outputs,file_evaluation);
    result_angle = (float)((360*reaction_time)*(neuron_outputs[1]-neuron_outputs[0])*-max_velocity)/(float)(2*M_PI*car_length);
    ...
    // if(generation>0){
    //     fprintf(file_tests,"Name: %d, outputs: %.3f %.3f, velocity:%.2f %.2f, angle: %.1f, chose_points: %d, real_outputs: %d, fitness:%d \n",genome->name, neuron_outputs[0],neuron_outputs[1],neuron_outputs[0]*max_velocity,neuron_outputs[1]*max_velocity,result_angle,choose_point,outputs,fitness);
    // }
    return fitness;
}
void create_false_fitness(unsigned char (*inputs)[length][4],int *output, int (*intensities),FILE *file_inputs, int name){
    ...
    for(int i=0; i<height; i++){
        for(int j=0;j<length; j++){
            ...
            if(color<color_propability){
                ...
                inputs[i][j][0] = ((float)rand()/(float)RAND_MAX)*255; 
                inputs[i][j][1] = ((float)rand()/(float)RAND_MAX)*255; 
                inputs[i][j][2] = ((float)rand()/(float)RAND_MAX)*255;  
            }
            inputs[i][j][3] = j;
    ...
    fprintf(file_inputs,"intensities:\n");
    for(int i=0; i<length; i++){
        if(intensities[i] > intensities[max_iter]){
            max_iter = i;
        }
        fprintf(file_inputs," %d ",intensities[i]);
    }
    *output = max_iter;
    fprintf(file_inputs,"\n output:%d\n",*output);
}
    ...
    while(current_specie!=NULL){
        ...
        for(int i=0; i<population; i++){
            ...
            for(int j=0; j<tests_numbers; j++){
                create_false_fitness(inputs,&real_output,intensities,file_inputs,current_genome->name);
                fitness = evaluate_false_fitness(inputs,real_output,intensities,current_genome,sentence_outputs,j,file_evaluation, file_tests,generation);
...
void kill_individuals(Specie *specie){
    ...
    while(current_specie != NULL){
        ...
        current_specie->reproductivity += current_genome->rank + (age_importance*current_genome->age);
        // printf("\n");
        // printf("specie %d has reproductivity: %d \n",current_specie->identifier,current_specie->reproductivity);
...
void calculate_offspring(Specie *specie){
    ...
    while(current_specie != NULL){
        if(total_fitness!=0){
            current_specie->reproductions = floor((current_specie->fitness)/ratio);
        }else{
...
//---------------------------------------Selecting partner---------------------------------------------------
Genome* choose_genome(Specie *specie){
    struct Genome *current_genome = specie->first, *result_genome;
    int iter_reproductivity = 0;
    bool finded = false;
    int random_reproductivity = (((float)rand()/(float)RAND_MAX)*specie->reproductivity)+1;
    while(!finded){
        iter_reproductivity += current_genome->rank +(age_importance*current_genome->age);
        if(random_reproductivity <= iter_reproductivity){
            result_genome = current_genome;
            finded = true;
        }else{
            current_genome = current_genome->next;
        }
...
void select_partner(Specie *current_specie, Genome **partner1){
    struct Genome *current_genome;
    current_specie->reproductions--;
    current_genome = choose_genome(current_specie);
    current_genome->sons++;
...
Genome* asexual_reproduction(Genome *parent){
    struct Genome *new_child = create_new_genome();
    struct Gene *copied_gene, *current_gene, *previous_copied_gene;
        ...
        if(specie!=NULL){
            ...
            while(new_childs!=kills){
                if(current_specie->reproductions>0){
                    select_partner(current_specie, &partner1);
                    new_child = asexual_reproduction(partner1);
